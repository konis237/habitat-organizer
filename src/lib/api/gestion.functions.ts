...
export const listProprietes = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("proprietes").select("*").order("nom");
...
export const upsertPropriete = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    if (data.id) {
      ...
      const { error } = await context.supabase
        .from("proprietes")
        .insert({ nom: data.nom, adresse: data.adresse, montant_loyer: data.montant_loyer, statut: "libre" });
      if (error) throw new Error(error.message);
    }
    return { ok: true };
  });
...
export const deletePropriete = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (error) throw new Error("Suppression impossible : un contrat existe peut-être pour ce bien.");
    return { ok: true };
  });
...
export const listLocataires = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await context.supabase
      .from("locataires").select("*").order("nom");
    if (error) throw new Error(error.message);
    // Aussi contrats actifs pour badge
    const { data: actifs } = await context.supabase
      .from("contrats").select("locataire_id").eq("statut", "actif");
...
export const deleteLocataire = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: actifs } = await context.supabase
      .from("contrats").select("id").eq("locataire_id", data.id).eq("statut", "actif");
    if (actifs && actifs.length > 0) throw new Error("Locataire avec contrat actif — impossible de supprimer.");
...
export const getLocataireDetail = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: locataire } = await context.supabase
      .from("locataires").select("*").eq("id", data.id).single();
...
export const createContrat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("proprietes").update({ statut: "occupe" }).eq("id", data.propriete_id);
    ...
    if (prochaine <= new Date()) {
      ...
      await context.supabase.from("loyers").insert({
        contrat_id: contrat.id, periode, montant: data.loyer_mensuel, date_echeance: prochaineISO,
      });
      if (data.eau_mensuelle > 0) {
        await context.supabase.from("factures_eau").insert({
          contrat_id: contrat.id, periode, montant: data.eau_mensuelle, date_echeance: prochaineISO,
        });
      }
    }
    return { ok: true };
...
export const cloturerContrat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: c } = await context.supabase
      .from("contrats").select("propriete_id").eq("id", data.id).single();
    await context.supabase.from("contrats").update({ statut: "cloture" }).eq("id", data.id);
    if (c) await context.supabase.from("proprietes").update({ statut: "libre" }).eq("id", c.propriete_id);
    return { ok: true };
  });
...
export const genererEcheancesMois = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data: contrats } = await context.supabase
      .from("contrats").select("*").eq("statut", "actif");
    let created = 0;
    for (const c of contrats ?? []) {
      ...
      const { error: e1 } = await context.supabase.from("loyers").upsert(
      ...
      if (Number(c.eau_mensuelle) > 0) {
        await context.supabase.from("factures_eau").upsert(
...
export const enregistrerPaiement = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: ech } = await context.supabase
      .from(table).select("*").eq("id", data.document_id).single();
    ...
    const { error: e1 } = await context.supabase.from(table)
      .update({ montant_paye: nouveauPaye, statut }).eq("id", data.document_id);
...
export const listPaiements = createServerFn({ method: "GET" })
  ...
  .inputValidator((d) => z.object({
    debut: z.string(), fin: z.string(),
    type: z.enum(["tous", "loyer", "eau"]).default("tous"),
  }).parse(d))
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("paiements").select("*")
      .gte("date_paiement", data.debut).lte("date_paiement", data.fin)
    ...
    const enriched = await Promise.all((paiements ?? []).map(async (p) => {
      ...
      const { data: doc } = await context.supabase.from(table)
        .select("periode, contrat:contrats(locataire:locataires(nom, prenom), propriete:proprietes(nom))")
        .eq("id", p.document_id).single();
      return { ...p, doc };
    }));
    return enriched;
  });
  ...
  .handler(async ({ context }) => {
    const fetchAll = async (table: "loyers" | "factures_eau", type: "loyer" | "eau") => {
      const { data } = await context.supabase
        .from(table)
        .select("id, periode, montant, montant_paye, reste, date_echeance, statut, contrat:contrats(propriete:proprietes(nom), locataire:locataires(nom, prenom))")
        .neq("statut", "paye")
        .order("date_echeance");
      return (data ?? []).map((r) => ({ ...r, type }));
    };
    const [loyers, eau] = await Promise.all([fetchAll("loyers", "loyer"), fetchAll("factures_eau", "eau")]);
    return [...loyers, ...eau].sort((a, b) =>
      new Date(a.date_echeance as string).getTime() - new Date(b.date_echeance as string).getTime(),
    );
  });
...
export const dashboardData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ count: nProp }, { count: nLoc }, { count: nContrats }, loyersMois, eauMois, impayesLoyers, impayesEau, prochaines, derniersPaiements] = await Promise.all([
      ...
      s.from("loyers")
        .select("id, montant, reste, date_echeance, contrat:contrats(propriete:proprietes(nom), locataire:locataires(nom, prenom))")
        .gte("date_echeance", todayISO).lte("date_echeance", sevenISO).neq("statut", "paye"),
      s.from("paiements").select("*").order("date_paiement", { ascending: false }).limit(5),
    ]);

    const sum = (rows: { montant?: number | string | null; reste?: number | string | null }[] | null, key: "montant" | "reste") =>
...
export const revenusData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [loyersM, eauM, impLoyers, impEau, paiementsAnnee, loyersDus, eauDus] = await Promise.all([
      ...
      s.from("loyers").select("reste").neq("statut", "paye"),
      s.from("factures_eau").select("reste").neq("statut", "paye"),
      s.from("paiements").select("montant, date_paiement, type_document")
        .gte("date_paiement", debut).lte("date_paiement", fin),
      s.from("loyers").select("montant, periode")
        .gte("periode", debut).lte("periode", fin),
      s.from("factures_eau").select("montant, periode")
        .gte("periode", debut).lte("periode", fin),
    ]);

    const sum = (rows: { montant_paye?: number | string | null; reste?: number | string | null; montant?: number | string | null }[] | null, key: string) =>
      (rows ?? []).reduce((a, r) => a + Number((r as Record<string, unknown>)[key] ?? 0), 0);

    // Mensuel
    const months = Array.from({ length: 12 }, (_, i) => {
      const m = String(i + 1).padStart(2, "0");
      return { key: `${data.annee}-${m}`, label: new Date(data.annee, i, 1).toLocaleDateString("fr-FR", { month: "short" }) };
    });
    const monthly = months.map((m) => {
      ...
      return {
        mois: m.label, loy, eau, totalEnc, dusLoy, dusEau, totalDu,
