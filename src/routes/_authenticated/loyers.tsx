  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-4">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-muted text-xs uppercase text-muted-foreground">
                  <th className="px-4 py-3 text-left">Locataire</th>
                  <th className="px-4 py-3 text-right">Montant</th>
                  <th className="px-4 py-3 hidden sm:table-cell text-center">Statut</th>
                  ...
                  <tr key={r.id} className="border-b hover:bg-muted/50">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-muted-foreground">
                        {formatPeriode(r.periode)} • {r.contrat?.propriete?.nom}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-right font-mono">{formatFCFA(r.montant)}</td>
                    <td className="px-4 py-3 hidden sm:table-cell text-center">
                      <StatutBadge statut={r.statut} />
                      {r.isVirtual && <span className="text-blue-500 text-xs ml-1">(Avance)</span>}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        <Button size="sm" variant="ghost" onClick={() => setSelectedDetail(r)}>
      ...
      <Dialog open={!!selectedDetail} onOpenChange={() => setSelectedDetail(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Détails du Loyer</DialogTitle>
          </DialogHeader>
          {selectedDetail && (
            <div className="space-y-4">
              <div>
                ...
                <p className="font-medium">
                  {selectedDetail.contrat?.locataire?.prenom}{" "}
                  {selectedDetail.contrat?.locataire?.nom}
                </p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground">Période</p>
                <p>{formatPeriode(selectedDetail.periode)}</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm text-muted-foreground">Montant</p>
                  <p className="font-bold">{formatFCFA(selectedDetail.montant)}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Restant</p>
                  <p className="font-bold text-destructive">{formatFCFA(selectedDetail.reste)}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <Dialog open={!!paying} onOpenChange={() => setPaying(null)}>
        <DialogContent>
          ...
          {paying && (
            <p className="text-sm text-muted-foreground mb-4">{formatPeriode(paying.periode)}</p>
          )}
          <form onSubmit={handlePay} className="space-y-4">
            <div>
